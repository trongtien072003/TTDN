...
{
    public abstract class proj_ttWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static proj_ttWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(proj_ttWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<proj_ttDbContext> action)
        {
            using (var context = IocManager.Resolve<proj_ttDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<proj_ttDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<proj_ttDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<proj_ttDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<proj_ttDbContext, Task<T>> func)
        {
            T result;
